...
function SubProjectPage() {
  type TopicsType = { NEW?: string, DEAD?: string };
  ...
  return (
      <div className="project">
        {topics.map((topic, index) => (
            <div className={Object.keys(topic)[0] === "NEW" ? "project-new" : "project-dead"} key={index}>
              <div className="project-title">
                <span>{Object.values(topic)} 프로젝트</span>
              </div>
              <div className="project-list">
                {newList.map((info, index) => (
                    <ProjectList key={index} info={info} isDead={Object.keys(topic)[0] !== "NEW"}/>
                ))}
              </div>
            </div>
        ))}
